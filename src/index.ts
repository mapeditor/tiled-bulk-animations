...
const action_animation_clear = tiled.registerAction(`${config.name}_ClearFromSelection`, animation_clear);
...
function animation_create() {
    ...
    if (selected_animated_tiles.length) {
        ...
        if (!response) return;
        debug(`Overwriting ${selected_animated_tiles.length} tile(s)`);
    }

    const [tileset_selected_tiles, tileset_dimensions] = tileset_selection(tileset, selected_tiles);
...
function animation_clear() {
    ...
    if (selected_animated_tiles.length) {
        ...
        if (!response) return;
        debug(`Clearing ${selected_animated_tiles.length} tile(s) animations`);
    }
...
function animation_dialog_create(dialog: Dialog, tileset_selected_tiles: rect, tile_dimensions: ITilesetDimensions): IAnimationConfirmation {
    ...
    // Maximum frame counts at the default stride, per direction
    ...
    animation_direction.currentIndexChanged.connect((index: number) => {
        ...
        animation_frames_input.maximum = index === 0 ? max_h_frames
